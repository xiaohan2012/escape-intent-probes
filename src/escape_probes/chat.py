"""What every real backend needs: tool schemas and prompt rendering.
...
Probe position (b) is shared for the same reason but lives in `model.py`, next
to the wire-format helper it calls and to the fake backend that also needs it.
